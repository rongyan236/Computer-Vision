...
void ViBe_BGS::testAndUpdate(const Mat _image)
{
	...
	for(int i = 0; i < _image.rows; i++)
	{
		const uchar* data = _image.ptr<uchar>(i);
...
/******
	...
	for(int i = 0; i < _image.rows; i++)
	{
		for(int j = 0; j < _image.cols; j++)
		{
			 bg1[i][j] = samples[i][j][0];
		}
	}
	Mat bg( row, col, CV_8UC1, bg1);
	imshow("samples",bg);
***/
...
/****均值背景*******
	...
	for(int i = 0; i < _image.rows; i++)
	{
		for(int j = 0; j < _image.cols; j++)
		{
			 bg1[i][j] = (samples[i][j][0]+samples[i][j][1]+samples[i][j][2]+samples[i][j][3]+samples[i][j][4]+samples[i][j][5] 
						+samples[i][j][6]+samples[i][j][7]+samples[i][j][8]+samples[i][j][9])/10;
		}
	}
	Mat bg( row, col, CV_8UC1, bg1);
	imshow("samples",bg);

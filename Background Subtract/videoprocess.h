...
class FrameProcessor {
  ...
  public:
	// processing method
	virtual void process(cv:: Mat &input, cv:: Mat &output, cv::Mat &background)= 0;
};
...
class VideoProcess {
  ...
  private:
	  ...
	  // background display window name
	  std::string windowNameBackground;
  ...
  public:
	  ...
	  VideoProcess() : callIt(false), delay(-1), 
	  ...
	  void setFrameProcessor(void (*frameProcessingCallback)(cv::Mat&, cv::Mat&, cv::Mat&)) {
	  ...
	  // to display the background frames
	  void displayBackground(std::string wn) {
	    
		  windowNameBackground= wn;
		  cv::namedWindow(windowNameBackground);
	  }
	  ...
	  void run() {
		  ...
		  while (!isStopped()) {
			  ...
			  if (callIt) {
				...
				// process the frame
				if (process)
				    process(frame, output, background);
				else if (frameProcessor) 
					frameProcessor->process(frame,output,background);
			 ...
			 // if (outputFile.length()!=0)
			   ...
			   if (windowNameBackground.length()!=0) 
				  cv::imshow(windowNameBackground,background);

...
namespace bgslibrary
{
  ...
  void ForegroundMaskAnalysis::process(const long &frameNumber, const std::string &name, const cv::Mat &img_input)
  {
    if (img_input.empty())
      ...
      cv::imshow("MaskInputAnalysis",img_input);
    ...
    if (stopAt == frameNumber && img_ref_path.empty() == false)
    {
	  ...
	  std::cout<<"input type:"<<img_input.type()<<std::endl;
	  std::cout<<"ref type:"<<img_ref.type()<<std::endl;

	   std::cout<<"input size:"<<img_input.size()<<std::endl;
	  std::cout<<"ref size:"<<img_ref.size()<<std::endl;
